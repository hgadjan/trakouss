...
export class MediatorProvider {
    ...
    constructor(public http: Http,public constant: Constant, public angularFire: AngularFire, public backProvid: BackendProvider, public onlineProv: OnlineProvider, public events: Events, public toastCtrl: ToastController, public logProvid: LogProvider) {
        this.logProvid.log('init Mediator provider');
    }

    saveUserToDb(user:any){
        let cache:any = [];
        return this.http.post(this.constant._API.baseUrl+'/user',JSON.stringify(user,function(key, value) {
    ...
    initMediator() {
        ...
        this.events.subscribe(GlobalStatictVar.ONLINE_USER_EVENT, (loggedinUser:any) => {
    ...
    subscribeToMessages() {
        ...
        this.userMessagesRef.on('child_changed', function (element:any) {
    ...
    getChat(uid:any) {
        return this.backProvid.getChat(uid);
    }
    ...
    getMessages(idChat:number) {
        return this.backProvid.getMessages(idChat);
    }
    ...
    saveRecievedMessages(message: Message, senderUser: User) {
        this.logProvid.log('');
        var self = this;
        this.backProvid.getChat(message.to).then((data) => {
            if (data.res.rows.length > 0) {
                let item = data.res.rows.item(0);
                let chat = {
                    id: item.id,
    ...
    getUnreadMessages() {
        var self = this;
        this.userMessagesRef = this.onlineProv.getUserMessagesRef(this.loggedinUser.uid);
        this.userMessagesRef.on('value', function (snapshots:any) {
            snapshots.forEach(function (childSnapshot:any) {
                var childKey = childSnapshot.key;
