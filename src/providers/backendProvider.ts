...
export class BackendProvider {
    ...
    initBackend() {
        ...
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT,chat_id INTEGER, uid, onlineKey TEXT, fromUid TEXT , toUid TEXT, body TEXT, msgType TEXT, msgDate Integer, msgStatus Integer, sentStatus Integer, CONSTRAINT message_unique UNIQUE (uid,msgDate,body))", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS chats (id INTEGER PRIMARY KEY AUTOINCREMENT, uid TEXT, uid2 TEXT, datetime Integer,lastMsgText TEXT, lastMsgDate Integer, recieverName TEXT NOT NULL, recieverPhoto TEXT, notify Integer, CONSTRAINT name_unique UNIQUE (uid,uid2) )", []);
    ...
    getMessages(idChat:number) {
        ...
        return this.storage.executeSql(query, [idChat]);
    }

    getChats() {
        let query = 'SELECT * FROM chats ORDER BY lastMsgDate DESC';
    ...
    getChat(uid:any) {
        let query = 'SELECT * FROM chats WHERE uid2 = ? LIMIT 1;';
        return this.storage.executeSql(query, [uid]);
    }
    ...
    removeMessageQueue(messageId:any) {
